...
BASE_URL = "/api/v1/botfarm/users/{0}.acquireLock"
...
async def test_lock_user_success(client: AsyncClient, db_sessionmaker: async_sessionmaker, user_entity: UserModel):
    ...
    assert user_entity.locktime is not None
...
async def test_lock_user_bad_request_user_already_locked(client: AsyncClient, db_sessionmaker: async_sessionmaker, user_entity: UserModel):
    async with db_sessionmaker.begin() as session:
        session.add(user_entity)
...
async def test_lock_user_bad_request_with_bad_data(client: AsyncClient, bad_id: str):
    access_token = await create_token()
    client.cookies = {AUTH_COOKIE_KEY: access_token}
...
async def test_lock_user_unauthorized(client: AsyncClient):
    client.cookies = {}
...
async def test_lock_user_forbidden(client: AsyncClient):
